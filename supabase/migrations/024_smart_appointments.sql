-- =============================================================================
-- 024_smart_appointments.sql
-- Smart Appointment Management — consultants, booking columns, waiting list,
-- special days, appointment settings seed. Idempotent.
-- =============================================================================
...
-- Seed default consultants (idempotent by name)
...
-- Bookings smart-appointment columns
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Constrain booking_source when constraint missing
    IF NOT EXISTS (
...
-- Appointment settings (merge-safe JSON in settings table)
...
-- Race mitigation note:
-- Full EXCLUDE constraint on (consultant_id, tstzrange) is optional and deferred —
-- existing rows may lack consultant_id / duration. Conflict check is enforced in
-- application code (select overlapping then insert) with owner force override + audit.
