-- Phase G: guest_customers + guest-keyed wishlist/cart/views + guest_id FKs
-- Idempotent. Requires customers / wishlist_items from 028+.
...
-- =============================================================================
-- guest_carts (durable cart by guest_id)
-- =============================================================================
...
-- =============================================================================
-- wishlist_items: allow guest_id (customer_id nullable)
-- =============================================================================
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Drop legacy unique if present (recreate as partial indexes)
    IF EXISTS (
    ...
    -- Also drop unnamed unique on those columns if named differently
    BEGIN
      ALTER TABLE wishlist_items
        DROP CONSTRAINT IF EXISTS wishlist_items_customer_product_unique;
...
-- =============================================================================
-- guest_id on shop_orders / bookings / saved_designs / addresses / reviews
-- =============================================================================
DO $$
BEGIN
  IF EXISTS (
...
-- =============================================================================
-- RLS (service role bypasses; admin policies for dashboard)
-- =============================================================================
