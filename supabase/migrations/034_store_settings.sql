-- Sprint S1: Store Settings Module
-- Idempotent. Safe to re-run.
--
-- Architecture choice:
--   Store config lives as JSONB under settings.key = 'store' (existing table).
--   Payment providers are an array inside that JSON — NOT a separate table —
--   because launch needs COD + Coming Soon stubs only; secrets stay in env
--   (configured flags + secret_env_ref names only; never plaintext secrets).
--   Shipping/contact sync into settings.key = 'site' via app merge-safe PUT
--   so existing checkout / CMS / Footer continue to work unchanged.
