-- 035_product_management_core.sql
-- Sprint P1.1 — Product Management Core (dresses additive columns)
-- Safe to re-run. Does not drop or rewrite existing product data.
-- Dual-write contract: status ↔ is_available (published=true; draft/hidden=false).
...
-- Backfill status from legacy is_available before NOT NULL / CHECK
UPDATE dresses
SET status = CASE
...
-- Optional FK to categories for "collection" (prefer featured_collection rows in admin UI)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Ensure updated_at exists (already in core schema; idempotent)
ALTER TABLE dresses
